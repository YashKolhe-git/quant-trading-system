...
def _handle_all_releases(
    ...
    existing.handle_mutual_excludes(
        value, existing.all_releases, existing.only_final, "all_releases",
    )
...
def _handle_only_final(
    ...
    existing = _get_release_control(parser.values, option)
    existing.handle_mutual_excludes(
        value, existing.only_final, existing.all_releases, "only_final",
    )
...
use_new_feature: Callable[..., Option] = partial(
    ...
    action="append",
    default=[],
    choices=["fast-deps", "build-constraint", "inprocess-build-deps",]
    + ALWAYS_ENABLED_FEATURES,
...
index_group: dict[str, Any] = {
    ...
    "options": [index_url, extra_index_url, no_index, find_links, uploaded_prior_to,],
}

package_selection_group: dict[str, Any] = {
    "name": "Package Selection Options",
    "options": [pre, all_releases, only_final, no_binary, only_binary, prefer_binary,],
}
